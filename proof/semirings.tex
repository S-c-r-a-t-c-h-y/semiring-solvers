...
\begin{itemize}
  \item \S\ref{sec:2-cat} and \S\ref{sec:multicat} define the 2-categorical and
  multicategorical notions we need, mostly to fix definitions and notations.
  \item \S\ref{sec:operads} defines the notion of operad and a 2-functor used in the
  construction of free algebras (\propositionref{Mod(O,-) is a 2-functor}).
  \item \S\ref{sec:multihom} defines the notion of multi-homomorphism and a multi-category
  and multi-functor used in the construction of free algebras
  (\propositionref{Multi(TT,C) multi-category}).
  \item \S\ref{sec:yoneda} gives a multi-categorical representability theorem for
  adjunctions using the Yoneda structure (\propositionref{representability multi-categorical adjunctions}).
  \item \S\ref{sec:multihom-extension} proves the existence of a left adjoint to a multi-functor under
  certain conditions (\theoremref{left adjoint iff commutative}).
  \item \S\ref{sec:distributivity} defines the concept of translation and of distributive
  ...
  \item \S\ref{sec:fral} constructs the free algebra for the distributive combination
...
\input{multihom}
\input{yoneda}
\input{multihom-extension}
\input{distributivity}
\input{fral}
