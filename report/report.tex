...
\todo[inline]{probably find a better name}

\begin{proof}[Proposition~\ref{prop:fral_solving}]
We want to prove that $A \brak{l} \env_A = A \brak{r} \env_A$. The universal property of $FX$ gives a homomorphism $h : \underline{FX} \to \underline{A}$ such that $h \circ \env_{FX} = \env_A$.
...
Now we can finally prove our equality in $A$. $FX$ validates $\textrm{eq}$, so $FX\brak{l} \env_{FX} = FX\brak{r} \env_{FX}$. Since $h$ is a homomorphism and homomorphisms preserve equations, $A \brak{l} (h \circ \env_{FX}) = A \brak{r} (h \circ \env_{FX})$. But $h \circ \env_{FX} = \env_A$, therefore $A \brak{l} \env_A = A \brak{r} \env_A$.
\end{proof}
