...
Intuitively, the theorem says that given two free algebras for the additive and multiplicative part, composing them gives the free algebra for the distributive combination of the theories. This construction is effective in the sense of \secref{sec:challenge}.
...
The second and third point are both taken care of by the hypothesis of commutativity of the additive theory in Theorem~\ref{thm:fail}. For the second point, two constants commute with each other if they are equal. A commutative theory can therefore not have two or more distinct constants. For the third point, the abides equation is simply the commutativity hypothesis for $f = g = \ast$.

However the first point is not accounted for. Informally, idempotence duplicates information in a way incompatible with the intended distributive-law construction. Under the current hypothesis, idempotent operators are allowed. Therefore, we need to strengthen our hypothesis.

A known failure is the distributive combination of semi-lattices with probabilistic nondeterminism \parencite{varacca_example}. Let $\mathcal M = \BA$ be the theory of barycentric algebras (Definition~\ref{def:barycentric_algebras}), $\mathcal A = \SL$ be the theory of join-semilattices (Definition~\ref{def:join_semilattices}). Barycentric algebras have idempotent operators, therefore there cannot exist a distributive law $\lambda : \BA\;\SL \Rightarrow \SL\;\BA$. The free semilattice on a set $X$ is $F_\SL X = \mathcal P_f^+(X)$ the set of non-empty finite subsets of $X$, with union as join, and the free barycentric algebra on $X$ is $F_\BA X = \mathcal D_f(X)$ the set of finitely-supported probability distributions on $X$. However, the free algebra on $X$ for the distributive combination of the two is $\mathrm{Conv}_f(\mathcal{D}_f (X))$, the set of non-empty finitely generated convex subset of $\mathcal{D}_f(X)$, and not $\mathcal P_f^+ (\mathcal D_f (X))$, the set of non-empty finite subsets of $\mathcal D_f(X)$.
...
The construction of free extensions for distributive combinations is still ongoing work. We figured out that we need to find a new pullback that is suited for extensions, and that we need to find a new adjunction between the multicategory of extensions and the obvious forgetful functor.\todo{this is not actually correct stated as is, it mixes categories and functors}