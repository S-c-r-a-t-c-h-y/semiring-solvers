...
\begin{align*}
  \forall e \in \PolExpr, \varphi_{PE}(e) &= \varphi_P(\norm(e)) &\text{(correctness)}\\
  \forall a \in A, \varphi_{PE}(\mathfrak{T} (a)) &= a &~
\end{align*}
...
The type $\Pol$ is built so that equality can be checked syntacticaly: if two polynomials $P_1, P_2 \in \Pol$ are equal then so are their syntax tree. Therefore, equality of normal form comes for free and one only needs to prove the correctness criterion for the tactic to be correct.
...
Both the \textsc{Frex} solvers and the \ring tactic are sound and complete.

Any fral/frex is canonically isomorphic to the quotient fral/frex, and an effective fral/frex with a constructive universality proof has an effective isomorphism to the quotient setoid. n this way, a simplifier using an effective fral/frex is constructively sound and complete \parencite{idris_frex}.
...
\begin{table}[!h]
  ...
  & \multirow{2}{*}{\textbf{Mere}} & \textbf{Mere} & & \textbf{impossible} nontrivially & \textbf{impossible} nontrivially \\ \cline{3-6}
  &                                   & \textbf{Inv.} & & \textbf{impossible} nontrivially & \textbf{impossible} nontrivially \\ \cline{2-6}
...
\begin{itemize}
  ...
  \item $\mathfrak m_n^\sigma := \{f \in \N\brak{X_1, \dots, X_n} \mid f(0, \dots, 0) = 0 \}$
\end{itemize}
...
Abort.
  \end{code}
because \tt{\string~\string~} is not a part of the ring structure. This is to be expected when looking at how reflection work in \S\ref{sec:reflection}. Because of this, theories such as involutive semirings or involutive rings are supported by \textsc{Frex}, but can't be supported by the \ring tactic.

\subsubsection{}

\todo[inline]{talk about proof extraction and other features of frex}
