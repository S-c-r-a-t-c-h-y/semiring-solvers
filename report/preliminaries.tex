...
Monads are closely related to algebraic theories, and the construction of the distributive combinations in \S\ref{sec:construction} uses the notions of monads and distributive laws from a category-theoretical point of vue. We will assume basic knowledge about category theory, mainly about functors, natural transformations and adjunctions. One can refer itself to \textcite{maclane}.
...
\end{tikzcd}
  ...
  \end{tikzcd}
  \]
These equations ensure that the distributive law is compatible for both the units and the multiplications or $S$ and $T$. This law induces a composite monad $ST$ on $\mathcal{C}$ with unit $\eta^{ST} = \eta^S\eta^T$  and multiplication $STST \xrightarrow{S \lambda T} SSTT \xrightarrow {\mu^S \mu^T} ST$.

\todo[inline]{give example/intuition about what a distributive law does (example of the list monad over the Abelian group monad), and how this gives the product of the composite monad}