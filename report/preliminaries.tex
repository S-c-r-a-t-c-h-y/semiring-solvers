...
In the general setting, let $X$ be a set of variables, $\mathcal{T}$ be a theory, $\la A, \env_A \ra$ be a $\mathcal{T}$-model over $X$ and $\la FX, \env_{FX} \ra$ be a free $\mathcal{T}$-model over $X$. We have the following proposition


\begin{proposition}
  \label{prop:fral_solving}
  Let $\textrm{eq} := X \vdash l = r$ be an equation in context. 
  
  \noindent If the free algebra $FX$ validates $\textrm{eq}$ then $A$ validates $\textrm{eq}$.
\end{proposition}
