...
One important definition is that of the universal arrow.
...
\begin{definition}[Universal arrow]
  ...
  A universal arrow from $U$ to $c$ is a pair $(F c, \eta_c)$ where:
  \begin{itemize}
    \item $F c \in A$
    \item $\eta_c : UFc \to c$ is a morphism in $C$
  \end{itemize}
...
\begin{exemple}[Defining the product with an universal arrow]
  ...
  \quad\sim\quad
  \begin{tikzcd}
    \Delta c \arrow[r, "\eta_{(a, b)}"] \arrow[dr, "{(f, g)}"'] & \Delta (a \times b) \arrow[dashed, d, "{(\pi_a, \pi_b)}"] \\
    & (a, b)
  \end{tikzcd}
  $$
  We have $\eta_{(a, b)} = \langle f, g\rangle$.
\end{exemple}
...
\begin{preuve}
  Let $F(X)$ be a free $\pres$-model over $X$. We want to show that $(F(X), \eta_X)$ is a universal arrow from $X$ to $U$, where $\eta_X : X \to U(F(X))$ is the function that sends each variable in $X$ to its interpretation in $F(X)$.
  ...
  \[
  \begin{tikzcd}
    X \arrow[r, "\eta_X"] \arrow[dr, "\rho_A"'] & U(F(X)) \arrow[d, "U(h)"] \\
    & U(A)
  \end{tikzcd}
  \]
...
\subsection{Constructing some free algebras}
...
We pose $F_{\CSG} = (S X, (+))$ where $+$ is the operator defined above. 

We want to show that $F_{\CSG}$ is a free $\CSG$-model over $X$.

\medskip 

$\triangleright$ Let's first check that $F_{\CSG}X$ is indeed a $\CSG$-model. We need to check that the axioms of $\CSG$ are valid in $F_{\CSG}X$. This is straightforward and comes down to the commutativity and associativity of addition in $\N$.

\medskip

$\triangleright$ Let $A$ be a $\CSG$-model over $X$ with environment $\phi_A : X \to U(A)$. We want to show\footnote{I think this is it ?} that there exists a unique morphism $h$ of $\CSG$-models from $F_{\CSG}X$ to $A$ such that the following diagram commutes:
\[
\begin{tikzcd}
...
$\triangleright$ Preservation of the operator $+$: let $f, g \in S X$. We have:
\begin{align*}
...
$\triangleright$ Commutation of the diagram: let $x \in X$. We have:
\begin{align*}
...
$\triangleright$ Uniqueness: let $h : F_{\CSG}X \to A$ be a morphism of $\CSG$-models such that the diagram above commutes. We want to show that $h = \Sigma\phi_A$. Let $f \in S X$. We have:
\begin{align*}
...
Compared to the free commutative semigroup, we can rely on the order of the terms and the order of applying the operations. We'll use lists to represent the terms of the free semigroup.
...
We pose $$SX = \{l \in X^* \mid l \text{ is non-empty}\}$$
...
\begin{itemize}
  ...
  \item compute the free algebras for the semigroup (and commutative semigroup), then combine them and see if this equals the free algebra for the combination (also check that the distributive laws hold)
